  ...
  constructor(
    ...
    this.webSocketService.getMessage().subscribe(
      (message: any) => {
        ...
        } else if (message.event === 'taskDeleted' || message.event === 'taskUpdated') {
          if(message.assignedToUid === this.authService.getId() || message.userUid === this.authService.getId()){
    ...
    this.taskApiService.updateTaskStatus(this.token, task.taskUid, !task.done).subscribe(
      (res) => {
        ...
        const notification = {
          ...
          userUid: task.createdByUid,
          message: `La tâche "${task.description}" a été modifiée.`
        };
    ...
    this.taskApiService.deleteTask(this.token, task.taskUid).subscribe(
      ()=>{
        ...
        const notification = {
          event: 'taskDeleted',
          assignedToUid: task.assignedToUid,
          userUid: task.assignedToUid,
          message: `La tâche "${task.description}" a été supprimée.`
        };
        this.webSocketService.sendMessage(notification);
        this.fetchTasks();
      }
